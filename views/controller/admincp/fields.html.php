...
{if count($aFields)}
<form method="post" action="{url link='admincp.digitaldownload.fields'}">
	<table id="js_drag_drop" cellpadding="0" cellspacing="0">
		...
		<tr class="checkRow{if is_int($iKey/2)} tr{else}{/if}">
			...
			<td><input type="checkbox" name="id[]" class="checkbox" value="{$aItem.field_id}" id="js_id_row{$aItem.field_id}" /></td>
			<td class="t_center">
	...
	<div class="table_bottom">
		<input type="submit" name="delete" value="{_p('Delete selected')}" class="sJsConfirm delete button sJsCheckBoxButton disabled" disabled="true" />
	</div>
</form>
{/if}